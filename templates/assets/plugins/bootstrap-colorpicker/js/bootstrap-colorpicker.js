...
convert.rgb.ansi256 = function (args) {
	...
	// grey and white. normal palette only has 4 greyscale shades.
	if (r === g && g === b) {
		if (r < 8) {
			return 16;
		}
...
convert.rgb.hcg = function (rgb) {
	...
	return [hue * 360, chroma * 100, greyscale * 100];
};
...
convert.grey.rgb = function (args) {
	return [args[0] / 100 * 255, args[0] / 100 * 255, args[0] / 100 * 255];
};

convert.grey.hsl = convert.grey.hsv = function (args) {
	return [0, 0, args[0]];
};

convert.grey.hwb = function (grey) {
	return [0, 100, grey[0]];
};

convert.grey.cmyk = function (grey) {
	return [0, 0, 0, grey[0]];
};

convert.grey.lab = function (grey) {
	return [grey[0], 0, 0];
};

convert.grey.hex = function (grey) {
	var val = Math.round(grey[0] / 100 * 255) & 0xFF;
    ...
    value: function onUpdate(event) {
      ...
      if (this.options.showText) {
        ...
        if (event.color.isDark() && event.color.alpha > 0.5) {
          ...
          this.elementInner.css('color', 'grey');
        }
      }
    }
  }]);
...
var skippedModels = [
	...
	// grey conflicts with some method names, and has its own method defined.
	'grey',
...
Color.prototype = {
	...
	grey: getset('hcg', 2, maxfn(100)),

	white: getset('hwb', 1, maxfn(100)),
	wgrey: getset('hwb', 2, maxfn(100)),

	cyan: getset('cmyk', 0, maxfn(100)),
	magenta: getset('cmyk', 1, maxfn(100)),
	yellow: getset('cmyk', 2, maxfn(100)),
	grey: getset('cmyk', 3, maxfn(100)),
	...
	greyscale: function () {
		// http://en.wikipedia.org/wiki/greyscale#Converting_color_to_greyscale
		var rgb = this.rgb().color;
...
var ColorHandler = function () {
  ...
  _createClass(ColorHandler, [{
    ...
    value: function getFallbackColor() {
      ...
      if (!color.isValid()) {
        console.warn('The fallback color is invalid. Falling back to the previous color or grey if any.');
